  ...
  return check;
	...
	$(window).scroll(function(){
		...
		if(positionFixed === true){
			...
			$('.Mail').css({left:"1%",top:"100px",margin:"0"});
			$('.Github').css({left:"94%",top:"100px",margin:"0"});

			$('.Twitter').css({left:"1%",top:"300px",margin:"0"});
			$('.Linkedin').css({left:"94%",top:"300px",margin:"0"});
		}
		...
		if(num > $("#OneLine").offset().top+$("#OneLine").height() && onlyOnce===true && !(noAnimation) === true){
			...
			$(".Mail").animate({top:"+="+height+'px',left:'-=' + width + 'px'},1000,function(){
